...
import pe.mil.ejercito.microservice.controllers.contracts.IUserController;
...
public class UserController extends ReactorControllerBase implements IUserController {
    private final IUserDomainService service;

    public UserController(Response response, final IUserDomainService service) {
        super(response, "ProfileController");
        this.service = service;
    }

    @Override
    @GetMapping(path = FIND_ALL_USER_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<Object>> doOnFindAllExecute(@RequestParam(required = false) String status,
        ...
        final PageableDto pageableUser = PageableDto.builder().build();
        return this.service.getAllEntities(status, profile, person, limit, page, pageableUser)
    ...
    @Override
    @GetMapping(path = FIND_BY_ID_USER_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<Object>> doOnFindByIdExecute(@PathVariable(value = "userId") Long id) {
    ...
    @Override
    @GetMapping(path = FIND_BY_UUID_USER_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    @Override
    @PostMapping(path = CREATE_USER_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<Object>> doOnCreateExecute(@RequestBody @Valid UserDto dto) {
    ...
    @Override
    @PutMapping(path = UPDATE_USER_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<Object>> doOnUpdateExecute(@RequestBody @Valid UserDto dto) {
    ...
    @Override
    @DeleteMapping(path = DELETE_USER_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
