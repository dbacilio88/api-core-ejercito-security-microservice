...
import pe.mil.ejercito.microservice.controllers.contracts.IProfileController;
...
public class PersonController extends ReactorControllerBase implements IProfileController {
    private final IProfileDomainService service;

    public PersonController(Response response, final IProfileDomainService service) {
    ...
    @Override
    @GetMapping(path = FIND_ALL_PROFILE_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<Object>> doOnFindAllExecute(@RequestParam(required = false) String status,
                                                           @RequestParam(required = false, defaultValue = "10") String limit,
                                                           @RequestParam(required = false, defaultValue = "1") String page) {
        final PageableDto pageableProfile = PageableDto.builder().build();
        return this.service.getAllEntities(status, limit, page, pageableProfile)
                .flatMap(current -> super.response(ProcessResponse.success(new PageableResponse<>(current, MetadataDto.builder()
                        .pageable(pageableProfile)
                        .build()))))
    ...
    @Override
    @GetMapping(path = FIND_BY_ID_PROFILE_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<Object>> doOnFindByIdExecute(@PathVariable(value = "profileId") Long id) {
    ...
    @Override
    @GetMapping(path = FIND_BY_UUID_PROFILE_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    public Mono<ResponseEntity<Object>> doOnCreateExecute(@RequestBody @Valid ProfileDto dto) {
    ...
    @Override
    @PutMapping(path = UPDATE_PROFILE_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<ResponseEntity<Object>> doOnUpdateExecute(@RequestBody @Valid ProfileDto dto) {
    ...
    @Override
    @DeleteMapping(path = DELETE_PROFILE_PATH, produces = MediaType.APPLICATION_JSON_VALUE)
